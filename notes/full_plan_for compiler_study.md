...
> **대상**: MLIR 기반 AI 컴파일러(Legato) 개발 중인 풀타임 직장인
> **장기 목표**: `triton-lang/Triton-to-tile-IR`에 기여
> **학습 전략**: 컴파일러 수업 자료 + MLIR Toy + 실전 프로젝트 교차 학습 → 기여
...
한 프로젝트만 파는 게 아니라, 각 Phase에서 배우는 개념에 맞는 프로젝트를 골라서 읽는다.
...
```
              ...
              torch.compile     JAX jit        tf.function
                    │               │               │
                    ▼               ▼               ▼
              Triton TTIR      StableHLO          XLA HLO
              (tile-based)     (portability)    (Google internal)
                    │               │               │
           ┌───────┼───────┐       │               │
           ▼               ▼       ▼               ▼
    TTGIR→PTX    CUDA Tile IR   Linalg          LLVM IR
    (기존경로)     (새 경로)    (IREE 등)        (CPU/GPU)
           │               │       │
           ▼               ▼       ▼
         CUBIN          tileiras   IREE Runtime
```
...
| 활동 | 빈도 | 시간 | 내용 |
|------|------|------|------|
| 문서 읽기 | 주 3회 | 30~60분 | MLIR 공식 문서 + 참고 프로젝트 문서 |
| 코드 읽기 | 주 3회 | 30분 | MLIR → Triton → StableHLO 순서로 확장 |
| 직접 구현 | 주 3회 | 1시간+ | 작은 rewrite / pass / custom op |
| 요약 노트 | 주 1회 | 30분 | 배운 것, 헷갈린 것, 프로젝트 간 비교 포인트 |
...
이 자료들은 MLIR을 대체하는 트랙이 아니라, MLIR/Triton 코드를 읽을 때 필요한
...
- Triton / StableHLO / Triton-to-tile-IR에서 "실전에서 어떻게 커지는지"를 확인한다.

즉, 한 주 안에서 항상 세 층위를 연결한다:

1. 이론: 수업 PDF
2. 학습용 구현: Toy Tutorial
3. 실전 구현: Triton / StableHLO / Triton-to-tile-IR

---

## Phase 0: 환경 세팅 + 생태계 조감 (1~2주차)

### 목표
- MLIR 소스 빌드 + Triton-to-tile-IR 빌드
- AI 컴파일러 생태계의 전체 그림 파악
...
cd Triton-to-tile-IR && pip install -e .
```

**생태계 조감도 그리기:**
아래 질문에 답할 수 있을 정도로 각 프로젝트의 README를 읽는다:
- Triton vs cuTile: 둘 다 tile-based Python DSL인데 뭐가 다른가?
- StableHLO vs Triton IR: 둘 다 MLIR dialect인데 추상화 수준이 어떻게 다른가?
- XLA vs Triton: 컴파일 방식이 어떻게 다른가? (graph-level vs kernel-level)
- IREE vs TorchInductor: 둘 다 ML 모델을 하드웨어로 내리는데 접근이 어떻게 다른가?
...
- LLVM Discord → `#mlir`
- GPU MODE Discord (Triton 핵심 커뮤니티)
- OpenXLA Discord (StableHLO/XLA 커뮤니티)
- GitHub Watch: `triton-lang/Triton-to-tile-IR`, `triton-lang/triton`

### 산출물
- 빌드 성공
- "AI 컴파일러 생태계 조감도" 1장 (위 다이어그램을 자기 말로 다시 그리기)

---

## Phase 1: 컴파일러 기초 + MLIR Toy Tutorial + 실전 프로젝트 교차 (3~6주차)
...
|------|--------|--------------|---------|
| 3주차 | Ch1~2 AST, IR, ODS | 01, 09, 17 | Triton / StableHLO / cuda-tile op 정의 |
| 4주차 | Ch3 Rewrite | 02~07, 22 | Triton canonicalization, rewriteAssume |
| 5주차 | Ch4~5 Lowering | 11, 12 | TritonToCudaTile, StableHLO→Linalg |
| 6주차 | Ch6~7 LLVM lowering | 10 복습용 선택, 17 재복습 | 5개 파이프라인 비교 |
...
**실전 프로젝트 교차 읽기:**
- `NVIDIA/cuda-tile`의 `.td` 파일: 실제 CUDA Tile dialect의 op이 ODS로 어떻게 정의되는지
- `openxla/stablehlo`의 `stablehlo/dialect/StablehloOps.td`: 100개+ op이 어떻게 체계적으로 정의되는지. 특히 verifier와 type inference가 op 정의에 포함되는 방식
- Triton의 `include/triton/Dialect/Triton/IR/TritonOps.td`: `tt.load`, `tt.store`, `tt.dot` 같은 op 정의
...
**실전 프로젝트 교차 읽기:**
- Triton의 `RemoveLayoutConversions` pass: layout conversion을 제거하는 4-phase 알고리즘. 이건 AI 컴파일러에서 가장 중요한 최적화 중 하나
- Triton-to-tile-IR의 `rewriteAssume` pass: assume op을 CUDA Tile IR로 변환하는 rewrite
- StableHLO의 canonicalization 패턴들: `stablehlo/transforms/` 디렉토리
...
**실전 프로젝트 교차 읽기 (핵심 주차!):**
- **Triton-to-tile-IR의 `TritonToCudaTile.cpp`**: TTIR→CUDA Tile IR conversion. 이번 주 핵심
  - ConversionTarget 설정: 어떤 op이 legal/illegal인지
  - 개별 conversion 패턴 2~3개를 Toy Ch5와 구조 비교
- **Triton 메인의 `ConvertTritonToTritonGPU`**: TTIR→TTGIR conversion. 같은 구조, 다른 대상
- **StableHLO→Linalg lowering** (`stablehlo/transforms/StablehloLegalizeToLinalgPass.cpp`): 또 다른 스타일의 dialect conversion
...
**핵심 관찰:** Triton-to-tile-IR은 Triton의 5단계 파이프라인 중 TTGIR→LLVM→PTX를 CUDA Tile IR로 대체하는 "백엔드 교체" 프로젝트다. 이 패턴은 AI 컴파일러 세계에서 계속 반복된다 (새 하드웨어 → 새 백엔드).
...
## Phase 2: Custom Dialect + Bufferization + 심화 읽기 (7~9주차)
...
## Phase 4: Triton 생태계 심화 + Python 레이어 (13~14주차)
...
|-------|------|----------|-------------------|-------------|
| 0 | 1~2 | 빌드, 생태계 조감 | 클론, 구조 파악 | 전체 README 읽기 |
| 1 | 3 | Op 정의, ODS | CUDA Tile `.td` | StableHLO `.td`, Triton `.td` |
| 1 | 4 | Rewrite | `rewriteAssume` | Triton `RemoveLayoutConversions` |
| 1 | 5 | Dialect conversion | `TritonToCudaTile` (핵심) | `ConvertTritonToTritonGPU`, StableHLO→Linalg |
| 1 | 6 | 전체 파이프라인 | PTX vs TileIR 비교 | 5개 프로젝트 파이프라인 비교 |
| 2 | 7 | Custom dialect | `NVIDIA/cuda-tile` | StableHLO op 설계 |
| 2 | 8 | Lowering pass | 체계적 분석 | Triton TTGIR→LLVM, StableHLO→Linalg |
| 2 | 9 | Bufferization | unordered memory model | IREE bufferization, XLA buffer alloc |
...
1. **Triton-to-tile-IR PR 1개** — 테스트, 문서, 또는 간단한 conversion 패턴
...
**"AI 컴파일러 생태계를 이해하고, 이미 기여하기 시작한 사람"**으로 보인다.

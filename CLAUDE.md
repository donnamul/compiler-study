...
Upstream sources (`llvm-project`, `triton`, `Triton-to-tile-IR`, `cuda-tile`, `iree`, ...) live in a sibling workspace at `~/dev/compiler-sources/` and must **not** be checked out, mirrored, or referenced as if they were inside this repo. The `.gitignore` explicitly excludes them.
...
Active plan is **Stage 0~4** (no time deadlines). The learning endpoint is contributing to `triton-lang/Triton-to-tile-IR` (TTIR → CUDA Tile IR conversion backend). Source of truth: `notes/full_plan_for compiler_study.md` (v5.2). Stage details:

- `notes/stage1-toy-mlir.md` — Toy Tutorial Ch1~7 + parallel C++ track. Dialect Conversion mechanism is the load-bearing skill.
- `notes/stage2-ttir-source.md` — Triton TTIR / TritonGPU dialect (source side of the conversion).
- `notes/stage2-iree-deep-read.md` — (former Stage 2) IREE deep-read. **Reference only — not an active track in v5.2.**
- `notes/stage2_5-cuda-tile.md` — NVIDIA/cuda-tile dialect (target side of the conversion).
- `notes/stage2_7-triton-to-tile.md` — `Triton-to-tile-IR` repo production conversion pass (`TritonToCudaTile.*`, `rewriteAssume.*`).
- `notes/stage3-custom-dialect.md` — out-of-tree mini *conversion pass* (A→B), reframed for v5.2.
...
- Do not re-introduce the dropped tracks: 22-PDF mandatory lecture parallel, Kaleidoscope, Torch-MLIR torch-dialect bridge, 16-week deadline, mandatory blog/PR outputs, multi-project (StableHLO + IREE + Triton + cuTile) simultaneous comparison.
- The v5.2 learning path is **TTIR source (Stage 2) → cuda-tile target (Stage 2.5) → Triton-to-tile-IR production conversion pass (Stage 2.7) → own mini conversion pass (Stage 3)**. IREE deep-read is reference only, not on the critical path.
- The Triton-internals exclusion is *kernel-writer-perspective only*. The **IR-internal view** (TTIR / TritonGPU dialect definitions, conversion entry points, `TritonToCudaTile.*` patterns) is an explicit study target in v5.2. Do not refuse Triton-internals questions on the old "already covered" grounds — verify which perspective is being asked about.
- Already-covered ground that stays excluded: Triton autotuning, kernel porting (Helion/CuTe/CUTLASS/ROCm), `torch.compile` internals, `triton/python/` DSL frontend.
- For tile-related dialect-design questions, `NVIDIA/cuda-tile`'s ODS is the target-side reference; `Triton-to-tile-IR`'s `TritonToCudaTile.*` is the production-conversion-pass reference.
